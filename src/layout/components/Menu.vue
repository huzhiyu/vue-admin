<template>
  <div>
    <MenuItem :info="item" :key="item.key" v-for="item in menuList"/>
  </div>
</template>
<script lang="ts">
   ...
   */
  const formatMenuList = (list: Array<RouteItem>, parentKey?: string) => {
   ...
   */
  const handleMerge = (list: Array<LayoutMenuItem>, onlyMergeFirst: boolean) => {
    list = JSON.parse(JSON.stringify(list));
    const result: Array<LayoutMenuItem> = [];
    for (let i = 0; i < list.length; i++) {
      const item = list[i];
  ...
  export default class Menu extends Vue {
    ...
    mounted() {
      this.update();
      const getElementHeight = (name: string, defaultHeight = 0) => {
