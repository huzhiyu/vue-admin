<template>
  <div class="the-layout-header">
    <div class="the-layout-navbar flex">
      <div :class="['flex fcenter fvertical hamburger', { 'hamburger-active': layoutInfo.sidebarOpen }]"
           @click="onSwitch()">
      ...
      <div class="user-info-box flex fvertical">
        ...
        <button @click="onLogout()" class="logout flex fvertical">
          <svg-icon name="exit"/>
          <span>退出登录</span>
        </button>
      </div>
    ...
    <div class="the-layout-tags" v-if="layoutInfo.showTagsView">
      <Scrollbar>
        <router-link :class="['the-layout-tag', {'the-layout-tag-on': isActive(item)}]"
                     :key="index + item.path"
                     :to="{ path: item.path, query: item.query, params: item.params }"
                     v-for="(item, index) in layoutInfo.historyViews">
          <span>{{ item.meta.title }}</span>
          <i @click.prevent.stop="onRemove(index)" class="close">-</i>
        </router-link>
      </Scrollbar>
    </div>
  </div>
