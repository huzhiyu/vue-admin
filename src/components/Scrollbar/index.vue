<template>
  <div @mouseenter="updateThumbStyle()" @mouseleave="onLeave()" class="the-scrollbar" ref="el">
    <div :class="[vertical ? 'the-scroll-y' : 'the-scroll-x']" :style="wrapStyle" ref="wrap">
      <slot/>
    </div>
    <transition name="fade">
      <button :style="thumbStyle" class="the-scroll-thumb" ref="thumb" title="滚动条-摁住拖拽" v-show="showThumb"/>
    </transition>
  </div>
</template>
<script lang="ts">
  ...
  export default class Scrollbar extends Vue {
    ...
    private initThumbStyle() {
      if (this.vertical) {
        this.thumbStyle.right = this.thumbStyle.top = "0";
        this.thumbStyle.width = this.thumbSize + "px";
      } else {
        this.thumbStyle.bottom = this.thumbStyle.left = "0";
