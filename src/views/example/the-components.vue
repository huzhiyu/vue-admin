<template>
  <div class="the-component">
    ...
    <div class="flex">
      <div :key="index" style="margin-right: 16px;" v-for="(item, index) in pageData.uploadList">
        <UploadImage :autoHeight="true" :src="item.image" :uploadId="index" @change="getPicUrl" tip="提示：图片高度自适应"/>
      </div>
      <div>
        <UploadImage :maxSize="5"
                     :src="pageData.single"
                     @change="getSingleUrl"
                     height="200px"
                     tip="提示：图片宽高固定尺寸 300px * 200px；限制 5M 内"
                     width="300px"/>
      </div>
    </div>
    <p class="tips">横向滚动</p>
    <div class="list-x">
      <Scrollbar thumbColor="#42b983">
        <div :key="item" class="item" v-for="(item) in list"/>
      </Scrollbar>
    </div>
    <p class="tips">垂直滚动</p>
    <div class="list-y">
...
<script lang="ts">
  ...
  export default class Page5 extends Vue {
    ...
    getPicUrl(info: { id: any; src: string; }) {
      const index = info.id;
      this.pageData.uploadList[index].image = info.src;
    };

    getSingleUrl(info: { src: string; }) {
